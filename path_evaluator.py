...
class PathEvaluator:
    def __init__(self):
        ...
        # 算法通用调节参数 (供智能体动态调优)
        self.params = {
            'max_turn_angle': 135.0,
            'chaikin_iterations': 3,     # 狭窄地带可能需要增加平滑迭代
            'min_waypoint_dist': 8.0,    # 航点弹簧排斥距离
    ...
    def _calculate_ideal_min_distance(self):
        ...
        pts.extend(targets)
        pts.append(self.env.start_point) # 现在是首尾相连
...
if __name__ == "__main__":
    ...
    # 保持你原来的测试用例不变
    # 比如完美路线加了 [78.0, 20.0], [43.0, 3.0] 作为折返
    # 逆时针完美闭环标杆路线 (0撞墙，0漏打卡，0急转弯)
    path_perfect_loop = np.array([
        [43.0,  3.0],  # 0. 起点 (南门)
        [58.0,  3.0],  # 1. 向东避障
        [58.0, 14.0],  # 2. 【打卡 P4】
        [58.0, 25.0],  # 3. 【打卡 P3】
        [60.0, 25.0],  # 4. 微调进入东侧大走廊
        [60.0, 39.0],  # 5. 【打卡 P2】
        [66.0, 39.0],  # 6. 避开前方圆形障碍
        [66.0, 69.0],  # 7. 沿东侧走廊一路北上
        [54.0, 69.0],  # 8. 【打卡 P1】
        [50.0, 69.0],  # 9. 向西穿过教学楼缝隙
        [50.0, 81.0],  # 10. 准备去操场
        [63.0, 81.0],  # 11. 【打卡 playground】
        [63.0, 89.0],  # 12. 向上绕过北侧体育馆
        [43.0, 89.0],  # 13. 来到中轴线
        [43.0, 54.0],  # 14. 【打卡 lake】 (垂直南下)
        [28.0, 54.0],  # 15. 准备去西侧河流
        [28.0, 63.0],  # 16. 【打卡 river】
        [17.0, 63.0],  # 17. 避障转角
        [17.0, 46.0],  # 18. 【打卡 P8】
        [29.0, 46.0],  # 19. 钻入西侧两栋楼之间的垂直通道
        [29.0, 40.0],  # 20. 【打卡 P7】
        [29.0, 28.0],  # 21. 【打卡 P6】
        [25.0, 28.0],  # 22. 微调避让右侧建筑群
        [25.0, 13.0],  # 23. 
        [28.0, 13.0],  # 24. 【打卡 P5】
        [28.0,  3.0],  # 25. 回到南侧围墙
        [43.0,  3.0]   # 26. 终点闭环
    ...
    # 莽夫直线直接起点连起点，用来测试它会不会因为漏打卡被扣大分
    path_lazy_straight = np.array([[43.0,  3.0], [43.0, 3.0]])
    
    # 擦边狂魔加上回程
    path_margin_graze = np.array([[43.0,  3.0], [43.0, 27.0], [41.0, 29.0], [24.5, 29.0], [24.5, 71.0], [76.0, 71.0], [78.0, 73.0], [78.0, 90.0], [75.0, 93.0], [51.0, 94.0], [75.0, 20.0], [43.0, 3.0]])
    
    # 死亡折返跑加上回程
    path_sharp_turns = np.array([[43.0,  3.0], [43.0, 40.0], [22.0, 40.0], [78.0, 40.0], [78.0, 70.0], [30.0, 70.0], [51.0, 94.0], [30.0, 20.0], [43.0, 3.0]])
    
    # 漏打卡路线加上回程
    path_missed_target = np.array([[43.0,  3.0], [43.0, 27.0], [45.0, 29.0], [78.0, 29.0], [78.0, 70.0], [78.0, 90.0], [75.0, 93.0], [51.0, 94.0], [75.0, 20.0], [43.0, 3.0]])

    test_cases = [
        ("完美标杆路线", path_perfect_loop),
    ...
    path_to_draw = path_perfect_loop 
