...
# === 修改：继承 BasePlanner，确保 self.dim, self.lb, self.ub 等属性正常初始化 ===
class WOAPlanner(BasePlanner):
    ...
    def _initialize_population(self):
        ...
        top_30_percent = int(self.pop_size * 0.3) # === 新增：计算前30%的数量 ===
        ...
        for i in range(self.pop_size):
            ...
            if i < top_30_percent and self.num_waypoints >= 4:
                candidate_pts[1] = [22.0, 40.0] # index 1 即第 2 个控制点
                candidate_pts[3] = [78.0, 70.0] # index 3 即第 4 个控制点
    ...
    def optimize(self):
        ...
        score_history = []
        self.convergence_curve = [] # 确保有此属性
        
        top_30_percent = int(self.pop_size * 0.3)
        ...
        for t in range(self.max_iter):
            ...
            for i in range(self.pop_size):
                ...
                # === 新增：在每次迭代更新位置后，再次强制精英个体踩在目标点上 ===
                if i < top_30_percent and self.num_waypoints >= 4:
                    waypoints_temp[1] = [22.0, 40.0]
                    waypoints_temp[3] = [78.0, 70.0]
